...
```{r eval=FALSE}
# utils::install.packages("Signac") # Incompatible with current R version
remotes::install_version(package = 'Signac', version = package_version('1.13.0'))
BiocManager::install("Rsamtools")
BiocManager::install("EnsDb.Hsapiens.v86")
BiocManager::install("biovizBase")
BiocManager::install("BSgenome.Hsapiens.UCSC.hg38")
# utils::install.packages("TFMPvalue") # Incompatible with current R version
...
# Remove peaks in non-standard chromosomes (e.g., genom scaffolds, contigs)
...
# Add gene annotation information to the Seurat/Signac object for the corresponding genome
# Peaks are not annotated yet, only information for all genes added to the object for later
...
#### ChromatinAssay object is specialised to include motif information, gene annotation & genome information for open chromatin data
```{r}
...
#### i. Nucleosome banding signal (per cell): 
####    Lengths of all fragments derived from DNA wrapped around histone octamers,
####    Ratio of mononucleosomal to nucleosome-free fragments
#### ii. Transcriptional start site (TSS) enrichment score (per cell): 
####     Ratio of fragments centered at TSSs compared to fragments flanking TSSs
####    (see https://www.encodeproject.org/data-standards/terms/)
#### iii. Total number of fragments in peaks (per cell)
####      Indicates sequencing depth & complexity.
#### iv. Fraction of fragments in peaks (FRIP) (per cell)
####      Cells with low FRIP indicate artifacts, debris or low-quality peaks
#### v. Ratio reads in genomic blacklist regions
####    Reads mapped to these regions are likely represent technical artifacts
...
Signac::FragmentHistogram(
    object = pbmc_atac,
    assay = 'peaks'
  )
Signac::DensityScatter(
    object = pbmc_atac,
    x = 'nCount_peaks',
    y = 'TSS.enrichment',
    log_x = TRUE,
...
Seurat::VlnPlot(
    ...
    assay = 'peaks',
    features = base::c(
        'nCount_peaks', 'TSS.enrichment', 'blacklist_ratio',
...
#### Plot QC metrics individually
```{r}
# Plot distribution of nucleosome signal
graphics::hist(
    x = pbmc_atac$nucleosome_signal,
    breaks = 20,
...
graphics::hist(
    x = pbmc_atac$pct_reads_in_peaks,
    # breaks = 100,
...
pbmc_atac <- base::subset(
    ...
    subset = nucleosome_signal < 1.5 &
      TSS.enrichment > 4 &
      nCount_peaks > 10000 & # Only non-standard value
      nCount_peaks < 100000 &
...
#### Steps adjusted from scRNAseq to scATACseq to account for the low dynamic range of scATACseq data
#### In combination TF-IDF & SVD are known as latent semantic indexing (LSI) to embed cells in low-dimensional latent space.
```{r}
# Perform term frequency-inverse document frequency (TF-IDF) normalization
# Normalizes 1) across cells to mitigate sequencing depth differences between cells &
# across 2) peaks to boost importance of rare peaks
...
# Dimensionality reduction via singular value decomposition (SVD) based on the TF-IDF matrix
# Resembles in essence the PCA approach for scRNAseq data
...
### The first LSI component captures sequencing depth (technical variation), not biological variation.
### Shown here as strong anti-correlation between sequencing depth and LSI component across all cells.
```{r}
Signac::DepthCor(
    object = pbmc_atac,
    n = 30
...
#### Louvain algorithm used for clustering approach.
```{r}
...
```{r eval=FALSE}
pbmc_atac <- base::list.files(
      path = here::here("output_data"),
      full.names = TRUE
    ) %>%
  stringr::str_subset("pbmc_atac") %>%
  base::readRDS(
      file = .
    )
```
...
#### To help interpreting scATACseq clusters based on the activity of non-coding
#### gene regulatory regions, we assess the overall activity of all open chromatin
#### regions overlapping the genes across the genome by:
#### Quantifying the activity of each gene by summing the fragments intersecting 
...
#### Plot chromatin activities for our 4 immune effector genes 
```{r warning=FALSE}
genes <- base::c(
    'CD8A', 'CD4', 'IL1A', 'IL1B'
  )
...
### [4.2.] Transfer cell-type labels from scRNA-seq data via cross-modality integration
#### Integration based on correlations between chromatin activities of genes (i.e., scATACseq) & gene expression (i.e., scRNAseq)
#### Identifies biological states (anchors) matching between both data modalities & provides a matching or classification score.
#### see [4.11.] in "20241014_scRNAseq_analysis.Rmd" for 'pbmc_qc' dataset
```{r warning=FALSE}
...
#### Evaluate overlap between predicted and real labels
```{r eval=FALSE}
pbmc_atac@meta.data %>%
  dplyr::mutate(
      ovl = predicted.id == real.labels
    ) %>%
  dplyr::group_by(ovl) %>% 
...
## [5.]  Supplement / Appendix - Differential accessibility analysis
...
## [6.]  Supplement / Appendix - Motif analysis
...
## [7.]  Supplement / Appendix - scATAC+scRNA multiome analysis: Linking peaks to genes
...
# pbmc_atac <- SortIdents(pbmc_atac) # Only in Signac >= 1.16.0 
