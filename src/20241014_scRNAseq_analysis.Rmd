...
## [4.1.] Run analysis with subsampled data
...
## [4.3.] Read out metainfo & UMAP data
```{r}
...
## [4.4.] Average mean cluster positions for cluster labels
```{r}
cls <- meta_comb %>%
...
## [4.5.] Map quality metrics onto UMAP
```{r}
coef = 0.5
cols <- base::c(
...
## [4.6.] Cluster-specific quality metrics
```{r}
ggplot2::ggplot(
      data = meta_comb,
      mapping = aes(
...
## [4.7.] Run analysis without removing mitoochondrial contaminations
#### No parameters changed comapred to [1.3.] - [3.4.]
...
## [4.8.] Map mitochondrial contribution to UMAP
```{r}
...
## [4.9.] Mean mitochondrial contribution per cluster
...
Seurat::FetchData(
    object = pbmc_qc,
    vars = base::c("CD8A","umap_1","umap_2"),
...
## [5.4.] Transfer annotations via identified anchors
```{r}
...
## [5.6.] Label predicted IDs onto the UMAP dimension reduction
```{r}
pal <- base::c(
